<?php
  ...
  if(isset($_POST) && !empty($_POST)){
    ...
    if($_POST['admin_password'] !== $_POST['admin_confirm_password']){
      ...
      $email_check = "SELECT * FROM admin WHERE admin_email = '$admin_email' LIMIT 1 ";
      $result = mysqli_query($conn, $email_check);
      $admins = mysqli_fetch_assoc($result);

      if($admins['admin_email'] === $admin_email){
        echo "<script>alert('มีการเปิดใช้อีเมลนี้แล้ว');</script>";
      }else{
        $passwordenc = md5($admin_password);
        $confirm_passwordenc = md5($admin_confirm_password);
        $sql = "INSERT INTO admin (admin_name,admin_email,admin_password,admin_confirm_password,admin_img) 
                VALUES ('$admin_name','$admin_email','$passwordenc','$confirm_passwordenc','$filename')";
            ...
            <div class="col">
              <div class="p-5 bg-light align-items-center text-end shadow fs-5" style="border-radius: 2rem; ">
                <div class="row g-3 align-items-center">
                  ...
                  <div class="col-7">
                    <input type="text" class="w-100 rounded-pill p-2" placeholder="Enter Name" name="admin_name" required>
                  </div>
                  <br>
                  <div class="col-5">
                  ...
                  <div class="col-7">
                    <input type="text" class="w-100 rounded-pill p-2" placeholder="Enter E-Mail" name="admin_email" required>
                  </div>
                  <br>
                  <div class="col-5">
                  ...
                  <div class="col-7">
                    <input type="password" class="w-100 rounded-pill p-2" placeholder="Enter Password" name="admin_password" required>
                  </div>
                  <br>
                  <div class="col-5">
                  ...
                  <div class="col-7">
                    <input type="password" class="w-100 rounded-pill p-2" placeholder="Confirm Password" name="admin_confirm_password" required>
                  </div>
                  <br>
                  <div class="col-5">
                  ...
                  <div class="col-7">
                    <input type="file" class="w-100 rounded-pill p-2" name="admin_img" required>
                  </div>
                  <br>
                 
                  <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <a class="btn btn-warning" onclick="window.history.back()">ย้อนกลับ</a>
                    <button class="btn btn-success" type="submit" name="submit">เพิ่มข้อมูล</button>
